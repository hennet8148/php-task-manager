...
if (isset($_POST['sync_tasks'])) {
    ...
    if ($neck_id > 0) {
        // Verify NECK_ID exists in the NECK table
        $neck_check_query = "SELECT COUNT(*) AS count FROM NECK WHERE ID = $neck_id";
        $neck_check_result = $conn->query($neck_check_query);
        $neck_check_row = $neck_check_result->fetch_assoc();

        if ($neck_check_row['count'] > 0) {
            // NECK_ID exists; proceed with copying tasks
            $sync_query = "
            ...
            if ($conn->query($sync_query) === TRUE) {
                ...
                echo "<div class='error'>Error during task synchronization: " . $conn->error . "</div>";
            }
        } else {
            // NECK_ID does not exist
            echo "<div class='error'>Error: Neck ID $neck_id does not exist!</div>";
        }
    } else {
